...
class ClassFee(BaseModel):
    """
    Default fee for each class level - base pricing
    فیس پیشفرض برای هر صنف - قیمت پایه
    """
    
    fee_type = models.ForeignKey(FeeType, on_delete=models.CASCADE, related_name='class_fee_levels')
    class_level = models.ForeignKey('api.ClassLevel', on_delete=models.CASCADE, related_name='class_fees')
...
class StudentFeeAssignment(BaseModel):
    ...
    def create_all_for_student(cls, student):
        """
        Previously this auto-created assignments based on `ClassFee` entries.
        Since `ClassFee` has been removed, this method is intentionally a no-op
        to avoid unexpected automatic assignments. Use explicit assignment
        creation via API or admin instead.
        """
        return []
...
class FinanceLedger(BaseModel):
    ...
    @classmethod
    @transaction.atomic
    def create_payment_entry(cls, payment, description=None):
        """Create ledger entry for payment (Debit Cash, Credit Receivable)"""
        entries = []
        
        # Credit Receivable (reduce receivable)
        entries.append(cls.objects.create(
            ...
            entry_side='credit',
            description=description or f"Payment against invoices"
        ))
