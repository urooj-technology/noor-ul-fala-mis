export const shopRentalFa = {
  shopRental: "کرایه دکان",
  shopRentals: "کرایه های دکان",
  status: "وضعیت",
  allStatuses: "همه وضعیتها",
  usd: "USD - دلار آمریکا",
  addShop: "افزودن دکان",
  editShop: "ویرایش دکان",
  shopDetails: "جزئیات دکان",
  shopInformation: "اطلاعات دکان",
  
  // Shop Information
  shopNumber: "شماره دکان",
  name: "نام",
  location: "مکان",
  area: "مساحت",
  monthlyRent: "کرایه ماهانه",
  currency: "ارز",
  description: "توضیحات",
  shop: "دکان",
  selectShop: "انتخاب دکان",
  tenant: "مستأجر",
  selectTenant: "انتخاب مستأجر",
  tenants: "کرایه نشینان",
  fullName: "نام کامل",
  phone: "تلفن",
  email: "ایمیل",
  tazkiraNumber: "شماره تذکره",
  address: "آدرس",
  afn: "AFN - افغانی",
  selectStatus: "انتخاب وضعیت",
  selectRentalStatus: "انتخاب وضعیت کرایه",
  
  // Statuses
  statusOptions: {
    available: "در دسترس",
    rented: "کرایه شده",
    maintenance: "در حال تعمیر",
    reserved: "رزرو شده"
  },
  
  // Tenant Information
  tenantDetails: "جزئیات مستأجر",
  tenantInformation: "اطلاعات مستأجر",
  addTenant: "افزودن مستأجر",
  editTenant: "ویرایش مستأجر",
  
  // Rental Information
  rental: "کرایه",
  rentalDetails: "جزئیات کرایه",
  rentalInformation: "اطلاعات کرایه",
  addRental: "افزودن کرایه",
  editRental: "ویرایش کرایه",
  startDate: "تاریخ شروع",
  endDate: "تاریخ پایان",
  rentalStatus: "وضعیت کرایه",
  securityDeposit: "سپرده امنیتی",
  
  // Rental Statuses
  rentalStatusOptions: {
    active: "فعال",
    expired: "منقضی شده",
    cancelled: "لغو شده",
    renewed: "تمدید شده"
  },
  
  // Actions
  viewDetails: "مشاهده جزئیات",
  edit: "ویرایش",
  delete: "حذف",
  deleteShop: "حذف دکان",
  deleteTenant: "حذف مستأجر",
  deleteRental: "حذف کرایه",
  
  // UI Components
  manageShops: "مدیریت سوابق دکانها",
  manageTenants: "مدیریت سوابق کرایه نشینان",
  manageRentals: "مدیریت سوابق کرایه",
  searchShops: "جستجوی دکانها...",
  searchTenants: "جستجوی کرایه نشینان...",
  searchRentals: "جستجوی کرایه ها...",
  filterByStatus: "فیلتر بر اساس وضعیت",
  filterByRentalStatus: "فیلتر بر اساس وضعیت کرایه",
  clearFilters: "پاک کردن فیلترها",
  noShopsFound: "هیچ دکانی یافت نشد",
  noTenantsFound: "هیچ مستأجری یافت نشد",
  noRentalsFound: "هیچ کرایهای یافت نشد",
  tryAdjustingSearch: "لطفاً عبارت جستجو خود را تغییر دهید",
  addFirstShop: "برای شروع اولین دکان خود را اضافه کنید",
  addFirstTenant: "برای شروع اولین مستأجر خود را اضافه کنید",
  addFirstRental: "برای شروع اولین کرایه خود را اضافه کنید",
  loadingShops: "در حال بارگذاری دکانها...",
  loadingTenants: "در حال بارگذاری کرایه نشینان...",
  loadingRentals: "در حال بارگذاری کرایه ها...",
  
  // Summary
  shopStatistics: "آمار دکانها",
  totalShops: "مجموع دکانها",
  availableShops: "دکانهای در دسترس",
  rentedShops: "دکانهای کرایه شده",
  totalMonthlyRent: "مجموع کرایه ماهانه",
  activeRentals: "کرایه های فعال",
  expiringRentals: "کرایه های در حال انقضا",
  monthlyIncome: "درآمد ماهانه",
  
  // Validation
  validation: {
    shopNumber: "شماره دکان الزامی است",
    name: "نام الزامی است",
    location: "مکان الزامی است",
    monthlyRent: "کرایه ماهانه الزامی است",
    tenant: "مستأجر الزامی است",
    startDate: "تاریخ شروع الزامی است",
    endDate: "تاریخ پایان الزامی است",
    fullName: "نام کامل الزامی است",
    phone: "تلفن الزامی است",
    tazkiraNumber: "شماره تذکره الزامی است"
  },

  // Payments
  payments: "پرداختها",
  managePayments: "مدیریت سوابق پرداخت کرایه",
  addPayment: "افزودن پرداخت",
  editPayment: "ویرایش پرداخت",
  paymentDetails: "جزئیات پرداخت",
  referenceNumber: "شماره مرجع",
  amount: "مبلغ",
  paymentDate: "تاریخ پرداخت",
  period: "دوره",
  periodMonth: "ماه",
  periodYear: "سال",
  paymentStatus: "وضعیت پرداخت",
  paymentStatusOptions: {
    pending: "در انتظار",
    completed: "تکمیل شده",
    cancelled: "لغو شده",
    refunded: "بازگشت داده شده"
  },
  receipt: "رسید",
  hasReceipt: "رسید دارد",
  viewReceipt: "مشاهده رسید",
  viewCurrentReceipt: "مشاهده فعلی",
  remove: "حذف",
  enterAmount: "مبلغ را وارد کنید",
  enterDescription: "توضیحات را وارد کنید",
  selectRental: "انتخاب کرایه",
  searchPayments: "جستجوی پرداختها...",
  noPaymentsFound: "هیچ پرداختی یافت نشد",
  addFirstPayment: "برای شروع اولین پرداخت خود را اضافه کنید",
  loadingPayments: "در حال بارگذاری پرداختها...",
  deletePayment: "حذف پرداخت",
  paymentNotFound: "پرداخت یافت نشد",
  
  // Financial Info
  rentalInfo: "اطلاعات کرایه",
  monthlyPaymentStatus: "وضعیت پرداخت ماهانه",
  paidThisMonth: "پرداخت شده این ماه",
  remaining: "باقیمانده",
  paymentProgress: "پیشرفت پرداخت",
  maxAmount: "حداکثر مبلغ",
  monthFullyPaid: "ماه به طور کامل پرداخت شد",
  monthPendingPayment: "در انتظار پرداخت",
  transactionInfo: "اطلاعات تراکنش",
  transactionNumber: "شماره تراکنش",
  
  // Financial Summary
  financialSummary: "خلاصه مالی",
  totalPaid: "کل پرداخت شده",
  totalRemaining: "کل باقیمانده",
  paymentHistory: "تاریخچه پرداخت",
  
  // Additional
  selectPaymentStatus: "انتخاب وضعیت پرداخت",
  allPaymentStatuses: "همه وضعیتهای پرداخت",
  
  // New fields for details and print
  shopInfo: "اطلاعات دکان",
  tenantInfo: "اطلاعات مستأجر",
  rentalPeriod: "دوره کرایه",
  yearlySummary: "خلاصه مالی سالانه",
  totalPaidYear: "پرداخت شده امسال",
  totalRemainingYear: "باقیمانده امسال",
  totalExpectedYear: "منتظره امسال",
  monthsPaid: "ماههای پرداخت شده",
  monthsPending: "ماههای باقیمانده",
  monthlyBreakdown: "وضعیت پرداخت ماهانه",
  rent: "کرایه",
  paid: "پرداخت شده",
  unpaid: "پرداخت نشده",
  partial: "جزئی",
  progress: "پیشرفت",
  total: "مجموع",
  month: "ماه",
  selectMonths: "انتخاب ماهها",
  totalAmount: "مبلغ کل",
  totalPayment: "کل پرداخت",
  monthsSelected: "ماه انتخاب شده",
  perMonth: "در ماه",
  printed: "چاپ شده",
  signature: "امضا",
  generatedBy: "این سند توسط سیستم مدیریت کرایه دکانها تولید شده است",

  startDatePeriod: "دوره شروع اجاره",
  selectStartDatePeriod: "انتخاب دوره شروع",
  dateFrom: "از تاریخ",
  dateTo: "تا تاریخ",
  startDatePeriodOptions: {
    daily: "امروز",
    weekly: "این هفته",
    monthly: "این ماه",
    yearly: "امسال",
    custom: "بازه دلخواه",
  },
  printSelected: "چاپ انتخاب‌شده‌ها",
  rentalsSelected: "اجاره انتخاب شده",
  rentalReport: "گزارش اجاره دکان‌ها",
  totalRentals: "مجموع اجاره‌ها",
  year: "سال",
  exportExcel: "خروجی اکسل",
  exportingExcel: "در حال خروجی...",
  exportExcelHint: "این گزارش ستون‌های زیادی دارد. برای نمای بهتر از اکسل استفاده کنید.",
  printColumns: {
    shopNo: "شماره دکان",
    shopName: "نام دکان",
    location: "موقعیت",
    tenant: "مستأجر",
    phone: "تلفن",
    startDate: "تاریخ شروع",
    endDate: "تاریخ پایان",
    monthlyRent: "کرایه ماهانه",
    status: "وضعیت",
    paidYear: "پرداخت‌شده (سال)",
    remainingYear: "باقی‌مانده (سال)",
    monthsPaid: "ماه‌های پرداخت‌شده",
  },
  periodMonths: "ماه‌ها",
  reports: "گزارش‌ها",
  reportsDesc: "دوره زمانی را انتخاب کنید تا همه اجاره‌ها و پرداخت‌ها نمایش و چاپ شوند",
  reportFilters: "فیلترهای گزارش",
  reportFiltersDesc: "فقط مدت زمان را انتخاب کنید — همه اجاره‌ها و پرداخت‌های آن دوره نمایش داده می‌شوند",
  reportDatePeriod: "دوره گزارش",
  generateReport: "ایجاد گزارش",
  printReport: "چاپ گزارش",
  periodReportTitle: "گزارش دوره‌ای کرایه دکان‌ها",
  reportPeriod: "دوره",
  rentalsInPeriod: "اجاره‌ها در دوره",
  paymentsInPeriod: "پرداخت‌ها در دوره",
  paymentsInPeriodDesc: "پرداخت‌هایی که تاریخ پرداخت آن‌ها در دوره انتخاب‌شده است",
  noRentalsInPeriod: "اجاره‌ای در این دوره یافت نشد",
  noPaymentsInPeriod: "پرداختی در این دوره یافت نشد",
  totalPayments: "مجموع پرداخت‌ها",
  totalPaymentAmount: "مجموع مبلغ پرداخت‌ها",
};
