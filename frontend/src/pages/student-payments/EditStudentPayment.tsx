...
const EditStudentPayment = () => {
  ...
  // Original payment assignment ID (for update)
  const [originalAssignmentId, setOriginalAssignmentId] = useState<number | null>(null);

  const { data, isLoading: fetching } = useFetchObject<any>({
  ...
  // Initialize from fetched payment data
  useEffect(() => {
    if (data) {
      const assignment = data.assignment_details;
      if (assignment) {
        // Set level and student
        setSelectedLevel(assignment.class_level_details?.id?.toString() || '');
        setSelectedStudent({
          id: assignment.student_details?.id,
          full_name: assignment.student_details?.full_name || '',
          registration_number: assignment.student_details?.registration_number || '',
          class_level: assignment.class_level_details?.id,
          class_level_details: assignment.class_level_details,
          currency: assignment.student_details?.currency || 'AFN',
          total_paid: assignment.student_details?.total_paid || '0',
          remaining_balance: assignment.student_details?.remaining_balance || '0',
        });
        setOriginalAssignmentId(assignment.id);

        // Set form fields
        setPaymentDate(data.payment_date?.slice(0, 10) || new Date().toISOString().split('T')[0]);
        setPaymentYear(data.period_year?.toString() || new Date().getFullYear().toString());
  ...
  // Initialize payment entries from fee assignments
  useEffect(() => {
    if (feeAssignmentsData?.total_assignments) {
      ...
      const entries: PaymentEntry[] = assignments.map((a: FeeAssignment) => {
        // If this is the original assignment, pre-populate with payment data
        const isOriginal = a.id === originalAssignmentId;
        const paymentData = isOriginal ? data : null;
        ...
        return {
          ...
          selected_months: paymentData?.period_months && Array.isArray(paymentData.period_months)
            ? paymentData.period_months.map((m: string) => String(m).padStart(2, '0'))
            : paymentData?.period_month
            ? [String(paymentData.period_month).padStart(2, '0')]
            : [],
  ...
  }, [feeAssignmentsData, originalAssignmentId, data]);
  ...
  // Handle success - navigate only, toast is handled by useUpdate
  useEffect(() => {
    if (isSuccess) {
  ...
  const handleSubmit = () => {
    ...
    if (validEntries.length === 0 || !id) {
      return;
    }

    // For edit, we only update the first valid entry (single payment edit)
    const entry = validEntries[0];

    const formDataObj = new FormData();
    formDataObj.append('assignment', entry.assignment_id.toString());
    formDataObj.append('amount', entry.payment_amount);
    formDataObj.append('payment_date', paymentDate);
    formDataObj.append('payment_status', paymentStatus);
    formDataObj.append('period_year', paymentYear);
    formDataObj.append('currency', entry.currency);
    formDataObj.append('period_months', JSON.stringify(entry.selected_months));
    if (referenceNumber) formDataObj.append('reference_number', referenceNumber);
            ...
            {totalPayment > 0 && (
              <div className="mt-6 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                      {t('student-payments.paymentStatus')}
                    </Label>
                    <select
                      value={paymentStatus}
                    ...
                    >
                      <option value="pending">{t('student-payments.status.pending')}</option>
                      <option value="completed">{t('student-payments.status.completed')}</option>
                      <option value="cancelled">{t('student-payments.status.cancelled')}</option>
                      <option value="refunded">{t('student-payments.status.refunded')}</option>
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
