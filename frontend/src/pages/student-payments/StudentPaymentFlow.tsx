    ...
    try {
      const paymentPromises = validEntries.map((entry) => {
        const payload = {
          ...
          allocation_mode: 'per_fee',
          allocations: {
            [entry.assignment_id]: entry.payment_amount,
          },
        };
        return fetch('/api/student-payments/', {
          method: 'POST',
      ...
      toast({
        ...
        description: `${validEntries.length} ${t('student-payments.paymentCount', 'payment(s)')} created successfully`,
      });
      ...
      toast({
        title: t('common.error'),
        description: error.response?.data?.error || t('student-payments.failedToCreatePayments', 'Failed to create payments'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="container mx-auto py-6 space-y-6 max-w-screen-2xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-base font-bold">{t('student-payments.paymentFlow', 'Student Payment Flow')}</h1>
      </div>
...
export default StudentPaymentFlow;